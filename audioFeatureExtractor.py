...
class AudioFtExt:
    ...
    def convertAudioToData(self):
        if self.duration_time is None:
            ...
            self.duration_time = self.audio_data / self.sampling_rate
        else:
            self.audio_data, self.sampling_rate = librosa.load(self.audio_path, sr=self.sampling_rate,
    ...
    # ARG 1: Guessed tempo. For better prediction
    def getRhythmData(self, beats_per_min=60):
        self.tempo, self.beat_data = librosa.beat.beat_track(self.audio_data, sr=self.sampling_rate,
